...
void Init(Reversi* reversi, Mode mode)
{
	...
	SetBoard(reversi, 3, 3, BLACK);
	SetBoard(reversi, 4, 4, BLACK);
	SetBoard(reversi, 3, 4, WHITE);
	SetBoard(reversi, 4, 3, WHITE);
	reversi->turn = BLACK;
}
// 次のターンへ
...
void DrawScreen(Reversi* reversi, Vector2 pos, DrawStatus stat)
{
	...
	case RESULT:
	{
		...
		// 「～の勝ちです」を表示します
		// 
		printf("%s%d-%s%d\n", getTurnName(BLACK), blackCount, getTurnName(WHITE), whiteCount);
